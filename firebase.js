...
async function registerWithEmail(accountId, displayName, password, email=null){
  ...
  try {
    ...
    const profile = {
      uid, accountId: accountId.toLowerCase(), displayName,
      email: email||null, googleLinked: false,
      createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      highScore:0, coins:0, medals:0, voidols:0, level:1, exp:0, rankings:[]
    };
...
async function completeGoogleRegistration(accountId, displayName){
  ...
  const profile = {
    uid, accountId:accountId.toLowerCase(), displayName,
    email:currentUser.email, googleLinked:true,
    createdAt:firebase.firestore.FieldValue.serverTimestamp(),
    highScore:0, coins:0, medals:0, voidols:0, level:1, exp:0, rankings:[]
  };
  const batch = fbDb.batch();
...
function getPrevWeekKey(){
  ...
  const day = d.getDay() === 0 ? 6 : d.getDay() - 1;
  const monday = new Date(d);
  monday.setDate(d.getDate() - day - 7);
  return `${monday.getFullYear()}-W${String(monday.getMonth()+1).padStart(2,'0')}${String(monday.getDate()).padStart(2,'0')}`;
}
...
async function saveOnlineRanking(score){
  ...
  try {
    ...
    if(!allDoc.exists || score > (allDoc.data().score || 0)){
      batch.set(allRef, base);
    }

    // 日別
    const dayKey = getDateKey();
    const dayRef = fbDb.collection('rankingsDaily')
                       .doc(`${dayKey}_${currentUser.uid}`);
    const dayDoc = await dayRef.get();
    if(!dayDoc.exists || score > (dayDoc.data().score || 0)){
      batch.set(dayRef, {...base, dateKey: dayKey});
    }

    // 週別
    const weekKey = getWeekKey();
    const weekRef = fbDb.collection('rankingsWeekly')
                        .doc(`${weekKey}_${currentUser.uid}`);
    const weekDoc = await weekRef.get();
    if(!weekDoc.exists || score > (weekDoc.data().score || 0)){
      batch.set(weekRef, {...base, weekKey});
    }

    // 月別
    const monthKey = getMonthKey();
    const monthRef = fbDb.collection('rankingsMonthly')
                         .doc(`${monthKey}_${currentUser.uid}`);
    const monthDoc = await monthRef.get();
    if(!monthDoc.exists || score > (monthDoc.data().score || 0)){
      batch.set(monthRef, {...base, monthKey});
    }
...
async function fetchOnlineRanking(){
  try {
    const snap = await fbDb.collection('rankings')
...
async function checkAndGrantRankingRewards(){
  if(!currentUser) return;
  try {
    const claimedDoc = await fbDb.collection('rewardsClaimed')
...
function showRankingRewardPopup(rewards){
  ...
  content.innerHTML = rewards.map(r => {
    ...
    const periodLabel = r.type === 'weekly'
      ? `${r.period}の週`
      : `${r.period}`;
    let rewardText = '';
    if(r.reward.voidols) rewardText += `💵 ${r.reward.voidols} ボイドル `;
    if(r.reward.medals)  rewardText += `🔘 ${r.reward.medals.toLocaleString()} メダル `;
    if(r.reward.coins)   rewardText += `🪙 ${r.reward.coins.toLocaleString()} コイン`;
    return `
      <div style="background:rgba(0,40,80,.7);border:2px solid #ffd700;
        ...
        <div style="color:#ffd700;font-size:14px;margin-bottom:8px">
          ${periodLabel} ${typeLabel}ランキング
        </div>
        <div style="color:#fff;font-size:28px;font-weight:900;margin-bottom:10px">
          🏆 ${r.rank}位
        </div>
        <div style="color:#0ff;font-size:16px;font-weight:900">
          ${rewardText}
        </div>
...
async function redeemSpecialCode(code){
  ...
  try {
    ...
    let rewardText = '';
    if(data.voidols) rewardText += `💵 ${data.voidols} ボイドル `;
    if(data.coins)   rewardText += `🪙 ${data.coins.toLocaleString()} コイン `;
    if(data.medals)  rewardText += `🔘 ${data.medals.toLocaleString()} メダル`;
    return {success:true, reward:rewardText.trim()};
...
fbAuth.onAuthStateChanged(async (user) => {
  if(user){
    ...
    if(doc.exists){
      ...
      playerData.highScore = currentUserProfile.highScore || 0;
      playerData.coins     = currentUserProfile.coins     || 0;
      playerData.medals    = currentUserProfile.medals    || 0;
      playerData.voidols   = currentUserProfile.voidols   || 0;
      playerData.level     = currentUserProfile.level     || 1;
      playerData.exp       = currentUserProfile.exp       || 0;
      playerData.rankings  = currentUserProfile.rankings  || [];
      playerData.nickname  = currentUserProfile.displayName;
      onLoginSuccess(currentUserProfile);
      // ログイン時に報酬チェック
