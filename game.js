...
function addOrbs(a){
  ...
  if(selectedBonuses.orb) amount *= 1.25;
  orbs += amount;
  updateMissionProgress('piece', a); // orb→piece統一
...
function moveLevelUp(){
  ...
  document.getElementById('levelUpTitle').textContent = `Move ${moveLevel}!`;
...
function update(){
  ...
  if(dx||dy){
    ...
    player.x += (dx/len)*player.speed*lvl.speed*dt;
    player.y += (dy/len)*player.speed*lvl.speed*dt;
  }
  ...
  enemies = enemies.filter(e => {
    if(e.isBoss){
      ...
      if(e.fireTimer <= 0 && Math.random() < fireChance){
        ...
        else { const a=Math.atan2(player.y-e.y,player.x-e.x); for(let i=0;i<3;i++) spawnBullet(e.x,e.y,a+(i-1)*0.15,210,true,'#ff00ff'); }
    ...
    for(let i = bullets.length-1; i >= 0; i--){
      ...
      if(Math.sqrt(ddx*ddx+ddy*ddy) < e.size*0.6+b.size){
        const dmg = lvl.dmg * diffMult.playerDmg;
        e.hp -= dmg;
        bullets.splice(i, 1);
        spawnExp(b.x, b.y, '#0cf', 4);
        updateMissionProgress('damage', dmg);
        if(e.hp <= 0){
...
function gameOver(){
  ...
  if(!tutorialMode){
    const levelMult = (() => {
      ...
      if(lv>=90) return 1.6; if(lv>=70) return 1.5; if(lv>=50) return 1.4;
      if(lv>=30) return 1.3; if(lv>=16) return 1.2; if(lv>=6)  return 1.1;
      return 1.0;
    })();
    const scoreBonusAdd = selectedBonuses.score ? 0.5 : 0;
    const totalMult  = levelMult + scoreBonusAdd;
    ...
    let multDetail = `×${levelMult.toFixed(1)}（LV補正）`;
    if(selectedBonuses.score) multDetail += ` ＋0.5（スコアボーナス）＝ ×${totalMult.toFixed(1)}`;
    ...
    if(selectedBonuses.exp && bonusExpMult > 1) expText += `（基本 ${baseExp} × ${bonusExpMult}倍）`;
    document.getElementById('expDisplay').textContent = expText;

    let coinText  = `+${gameCoins.toLocaleString()}🪙`;
    if(selectedBonuses.coin  && finalCoinMult  > 1) coinText  += `（×${finalCoinMult}倍）`;
    let medalText = `+${gameMedals.toLocaleString()}🔘`;
    if(selectedBonuses.medal && finalMedalMult > 1) medalText += `（×${finalMedalMult}倍）`;
  ...
  if(!tutorialMode && currentUser){
    const _finalScore = (() => {
      const lv = playerData.level;
      let lm = 1.0;
      if(lv>=90) lm=1.6; else if(lv>=70) lm=1.5; else if(lv>=50) lm=1.4;
      else if(lv>=30) lm=1.3; else if(lv>=16) lm=1.2; else if(lv>=6) lm=1.1;
      const add = selectedBonuses.score ? 0.5 : 0;
      return Math.floor(score * (lm + add));
    })();
